...
//      getAspect().addDependency(this).onChange(MemoryProfilerAspect.LIVE_ALLOCATION_STATUS, update);
  ...
  public long getAllocationTrackingElapsedTimeNs() {
    if (isTrackingAllocations()) {
      try {
        Transport.TimeResponse timeResponse = getStudioProfilers().getClient().getTransportClient()
          .getCurrentTime(Transport.TimeRequest.newBuilder().setStreamId(getSessionData().getStreamId()).build());
