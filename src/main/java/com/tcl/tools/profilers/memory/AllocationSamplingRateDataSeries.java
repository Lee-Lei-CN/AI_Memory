...
import com.tcl.tools.profilers.memory.AllocationSamplingRateDurationData;import org.jetbrains.annotations.NotNull;
...
public final class AllocationSamplingRateDataSeries implements DataSeries<AllocationSamplingRateDurationData> {
  ...
  private List<SeriesData<AllocationSamplingRateDurationData>> getLegacyData(long rangeMinNs, long rangeMaxNs) {
    ...
    MemoryProfiler.MemoryRequest.Builder dataRequestBuilder = MemoryProfiler.MemoryRequest
      .newBuilder()
      .setSession(mySession)
      .setStartTime(0)
      .setEndTime(Long.MAX_VALUE);
    List<MemoryProfiler.AllocationSamplingRateEvent> events =
    ...
    for (int i = 0; i < events.size(); i++) {
      MemoryProfiler.AllocationSamplingRateEvent currEvent = events.get(i);
      // Event is after our request window so skip the rest.
      ...
      MemoryProfiler.AllocationSamplingRateEvent nextRateEvent = i == events.size() - 1 ? null : events.get(i + 1);
