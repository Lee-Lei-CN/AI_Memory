...
import com.tcl.tools.profilers.memory.MemoryProfiler.Companion.hasOnlyFullAllocationTrackingWithinRegion
...
class LiveAllocationCaptureObject(
    ...
    private val callstackMap = TIntObjectHashMap<Memory.AllocationStack>()

    // Mapping from unsymbolized addresses to symbolized native frames
    private val nativeFrameMap = TLongObjectHashMap<Memory.NativeCallStack.NativeFrame>()
    ...
    private val allocationEventAdapter = object : EventAdapter<Memory.BatchAllocationEvents, Memory.AllocationEvent> {
        ...
        override fun getBatchEvents(startTimeNs: Long, endTimeNs: Long) =
            (if (stage.studioProfilers!!.ideServices.featureConfig.isUnifiedPipelineEnabled)
                ...
                getClient().getAllocationEvents(
                    MemoryProfiler.AllocationSnapshotRequest.newBuilder()
                        .setSession(session)
    ...
    private val jniReferenceEventAdapter =
        object : EventAdapter<Memory.BatchJNIGlobalRefEvent, Memory.JNIGlobalReferenceEvent> {
            override fun getTimestamp(event: Memory.JNIGlobalReferenceEvent) = event.timestamp
            override fun getEventList(batch: Memory.BatchJNIGlobalRefEvent) = batch.eventsList
            override fun getBatchEvents(startTimeNs: Long, endTimeNs: Long) =
                getEvents(
                    startTimeNs,
                    endTimeNs,
                    Common.Event.Kind.MEMORY_JNI_REF_EVENTS
                ) { it.memoryJniRefEvents.events }
    ...
    override fun getClassifierAttributes() =
        listOf(
            CaptureObject.ClassifierAttribute.LABEL,
            CaptureObject.ClassifierAttribute.ALLOCATIONS,
            CaptureObject.ClassifierAttribute.DEALLOCATIONS,
            CaptureObject.ClassifierAttribute.TOTAL_COUNT,
            CaptureObject.ClassifierAttribute.SHALLOW_SIZE,
            CaptureObject.ClassifierAttribute.SHALLOW_DIFFERENCE
        )

    override fun getInstanceAttributes() = listOf(
        CaptureObject.InstanceAttribute.LABEL,
        CaptureObject.InstanceAttribute.ALLOCATION_TIME,
        CaptureObject.InstanceAttribute.DEALLOCATION_TIME,
        CaptureObject.InstanceAttribute.SHALLOW_SIZE
    )
    ...
    private fun updateAllocationContexts(endTimeNs: Long) {
        if (contextEndTimeNs < endTimeNs) {
            fun <T> TIntObjectHashMap<T>.putIfAbsent(k: Int, v: T) {
                if (!containsKey(k)) {
                    put(k, v)
                }
            }

            fun <T> TLongObjectHashMap<T>.putIfAbsent(k: Long, v: T) {
                if (!containsKey(k)) {
                    put(k, v)
                }
            }
            for (contexts in getAllocationContexts(contextEndTimeNs, endTimeNs)) {
                ...
                contexts.classesList.forEach {
                    classDb.registerClass(
                        it.classId.toLong(),
                        Type.getType(it.className).className
                    )
                }
    ...
    private fun Memory.AllocationEvent.Allocation.getOrCreateInstanceObject() =
        instanceMap[tag] ?: classDb.getEntry(classTag.toLong()).let { entry ->
            ...
            LiveAllocationInstanceObject(
                this@LiveAllocationCaptureObject,
                entry,
                thread,
                callstack,
                size,
                heapId
            ).also { instanceMap.put(tag, it) }
        }

    private fun Memory.JNIGlobalReferenceEvent.getOrCreateJniRefObject() =
        instanceMap[objectTag]?.let { referencedObject ->
            referencedObject.getJniRefByValue(refValue) ?: JniReferenceInstanceObject(
                this@LiveAllocationCaptureObject,
                referencedObject,
                objectTag.toLong(),
                refValue
    ...
    private fun queryJavaInstanceSnapshot(snapshotTimeNs: Long, snapshotList: MutableList<InstanceObject>) =
        querySnapshot(snapshotTimeNs, snapshotList, allocationEventAdapter) { event, liveInstanceMap ->
            when (event.eventCase) {
                Memory.AllocationEvent.EventCase.ALLOC_DATA -> {
                ...
                Memory.AllocationEvent.EventCase.FREE_DATA -> {
                ...
                Memory.AllocationEvent.EventCase.CLASS_DATA -> {}
                else->{}
            }
        }
        ...
        if (enableJniRefsTracking) {
            querySnapshot(snapshotTimeNs, snapshotList, jniReferenceEventAdapter) { event, instanceMap ->
                when (event.eventType) {
                    Memory.JNIGlobalReferenceEvent.Type.CREATE_GLOBAL_REF -> {
                    ...
                    Memory.JNIGlobalReferenceEvent.Type.DELETE_GLOBAL_REF -> instanceMap.remove(event.refValue)
                        ?.let { refObject ->
    ...
    private fun <E> querySnapshot(
        snapshotTimeNs: Long, snapshotList: MutableList<InstanceObject>,
            ...
            queryDelta(startTimeNs, endTimeNs, jniReferenceEventAdapter) { event ->
                ...
                event.getOrCreateJniRefObject()?.let { refObject ->
                    when (event.eventType) {
                        ...
                        Memory.JNIGlobalReferenceEvent.Type.DELETE_GLOBAL_REF -> {
                            if (resetInstance) {
    ...
    private fun JniReferenceInstanceObject.setAllocEvent(event: Memory.JNIGlobalReferenceEvent) {
    ...
    private fun lookupThreadId(threadId: Int): ThreadId =
        if (threadId != 0) threadIdMap[threadId]!! else ThreadId.INVALID_THREAD_ID

    fun resolveNativeBacktrace(backtrace: Memory.NativeBacktrace?): Memory.NativeCallStack = when {
        backtrace == null || backtrace.addressesCount == 0 -> Memory.NativeCallStack.getDefaultInstance()
        else -> Memory.NativeCallStack.newBuilder().let { builder ->
            for (address in backtrace.addressesList) {
                if (!nativeFrameMap.containsKey(address)) {
                    val (module, offset) = getRegionByAddress(address)?.let {
                        Pair(
                            it.name,
                            it.fileOffset + (address - it.startAddress)
        ...
        if (stage.studioProfilers!!.ideServices.featureConfig.isUnifiedPipelineEnabled)
            client.transportClient.getEventGroups(
                buildEventGroupRequest(
                    ...
                    startTimeNs,
                    endTimeNs + QUERY_BUFFER_NS
                )
            )
                .getResultList { it.memoryAllocContexts.contexts }
        else
            getClient().getAllocationContexts(
    ...
    private fun <T> getEvents(
        startTimeNs: Long,
        endTimeNs: Long,
        event: Common.Event.Kind,
        extract: (Common.Event) -> T
    ): List<T> =
        client.transportClient.getEventGroups(
    ...
    private fun <T> List<T>.updateSeenTimestamp(timestamp: (T) -> Long) =
        stream().mapToLong(timestamp).max().ifPresent {
