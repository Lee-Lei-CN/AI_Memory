...
public interface InstanceObject extends ValueObject {
   ...
   */
  @Nullable
  default Memory.AllocationStack getAllocationCallStack() {
    return null;
  }
  ...
  default List<CodeLocation> getAllocationCodeLocations() {
    Memory.AllocationStack callStack = getAllocationCallStack();
    if (callStack != null && callStack.getFrameCase() == Memory.AllocationStack.FrameCase.FULL_STACK) {
      Memory.AllocationStack.StackFrameWrapper fullStack = callStack.getFullStack();
      if (!fullStack.getFramesList().isEmpty()) {
        List<CodeLocation> stackFrames = fullStack.getFramesList().stream()
          .map(stackFrame -> new CodeLocation.Builder(stackFrame.getClassName())
