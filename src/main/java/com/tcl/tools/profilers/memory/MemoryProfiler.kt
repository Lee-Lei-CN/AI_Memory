...
class MemoryProfiler(profilers: StudioProfilers) : StudioProfiler(profilers) {
  ...
  private fun importHprof(file: File) {
    fun makeInfo(start: Long, end: Long) = Memory.HeapDumpInfo.newBuilder().setStartTime(start).setEndTime(end)
    if (myProfilers.ideServices.featureConfig.isUnifiedPipelineEnabled) {
      ...
      legacyImport(file) { session, bytes, start, end ->
        myProfilers.client.memoryClient.importHeapDump(
          MemoryProfiler.ImportHeapDumpRequest.newBuilder()
            .setSession(session)
    ...
    import(file) { start, end ->
      makeEndedEvent(start, Common.Event.Kind.MEMORY_NATIVE_SAMPLE_CAPTURE) {
        setMemoryNativeSample(
          Memory.MemoryNativeSampleData.newBuilder()
                                .setStartTime(start)
                                .setEndTime(end))
      }
    }
  ...
  private fun importLegacyAllocations(file: File) {
    fun makeInfo(start: Long, end: Long) =
      Memory.AllocationsInfo.newBuilder().setStartTime(start).setEndTime(end).setLegacy(true).setSuccess(true)
    if (myProfilers.ideServices.featureConfig.isUnifiedPipelineEnabled) {
      import(file) { start, end ->
      ...
      legacyImport(file) { session, bytes, start, end ->
        myProfilers.client.memoryClient.importLegacyAllocations(
          MemoryProfiler.ImportLegacyAllocationsRequest.newBuilder()
            .setSession(session)
            .setInfo(makeInfo(start, end))
            .setData(ByteString.copyFrom(bytes))
            .build())
  ...
  companion object {
    ...
    fun hasOnlyFullAllocationTrackingWithinRegion(profilers: StudioProfilers,
      ...
      return samplingModes.size == 1 && samplingModes[0].value.currentRate.samplingNumInterval == BaseStreamingMemoryProfilerStage.LiveAllocationSamplingMode.FULL.value
    }

    @JvmStatic
    fun saveHeapDumpToFile(client: ProfilerClient,
                           session: Common.Session,
                           info: Memory.HeapDumpInfo,
    ...
    fun saveHeapProfdSampleToFile(client: ProfilerClient,
                                  session: Common.Session,
                                  info: Memory.MemoryNativeSampleData,
                                  outputStream: OutputStream) =
    ...
    fun getHeapDumpsForSession(client: ProfilerClient, session: Common.Session, rangeUs: Range,
                               profilerService: IdeProfilerServices
    ): List<Memory.HeapDumpInfo> =
      if (profilerService.featureConfig.isUnifiedPipelineEnabled)
        ...
        client.memoryClient.listHeapDumpInfos(
    ...
    fun getAllocationInfosForSession(client: ProfilerClient, session: Common.Session, rangeUs: Range,
                                     profilerService: IdeProfilerServices): List<Memory.AllocationsInfo> =
      if (profilerService.featureConfig.isUnifiedPipelineEnabled)
        getForSession(client, session, rangeUs, Common.Event.Kind.MEMORY_ALLOC_TRACKING) {
          ...
          if (info == Memory.AllocationsInfo.getDefaultInstance()) {
    ...
    fun trackAllocations(profilers: StudioProfilers,
                         session: Common.Session,
                         enable: Boolean,
                         responseHandler: Consumer<Memory.TrackStatus?>?) {
      val timeNs = profilers.client.transportClient
        .getCurrentTime(Transport.TimeRequest.newBuilder().setStreamId(session.streamId).build())
        .timestampNs
      if (profilers.ideServices.featureConfig.isUnifiedPipelineEnabled) {
        ...
        val response = profilers.client.memoryClient.trackAllocations(
          MemoryProfiler.TrackAllocationsRequest.newBuilder().setSession(session).setRequestTime(timeNs).setEnabled(enable).build())
