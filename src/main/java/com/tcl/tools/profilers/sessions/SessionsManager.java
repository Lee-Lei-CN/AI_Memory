 ...
 */
package com.tcl.tools.profilers.sessions;

import static com.tcl.tools.profilers.StudioProfilers.buildSessionName;
...
import com.tcl.tools.profilers.memory.AllocationSessionArtifact;
import com.tcl.tools.profilers.memory.HeapProfdSessionArtifact;
...
public class SessionsManager extends AspectModel<SessionAspect> {
  ...
  public void update() {
    if (myProfilers.getIdeServices().getFeatureConfig().isUnifiedPipelineEnabled()) {
      ...
      Profiler.GetSessionsResponse sessionsResponse =
        myProfilers.getClient().getProfilerClient().getSessions(Profiler.GetSessionsRequest.getDefaultInstance());
  ...
  private void updateSessions() {
    ...
    Transport.GetEventGroupsRequest request = Transport.GetEventGroupsRequest.newBuilder().setKind(Event.Kind.SESSION).build();
  ...
  private void updateSessionItemsByGroup(List<Transport.EventGroup> groups) {
    ...
    List<Transport.EventGroup> sortedGroups = Lists.newArrayList(groups);
    ...
    Collections.sort(sortedGroups, Comparator.comparing(Transport.EventGroup::getEventsCount, Comparator.reverseOrder())
    ...
    sortedGroups.forEach(group -> {
      ...
      if (startEvent.getSession().getSessionStarted().getType() != Common.SessionData.SessionStarted.SessionType.FULL &&
  ...
  public void beginSession(long streamId, @NotNull Common.Device device, @NotNull Common.Process process) {
    ...
    if (myProfilers.getIdeServices().getFeatureConfig().isUnifiedPipelineEnabled()) {
      assert streamId != 0;
      Commands.BeginSession.Builder requestBuilder = Commands.BeginSession.newBuilder()
      ...
      if (device.getFeatureLevel() >= AndroidVersion.VersionCodes.O &&
        ...
        requestBuilder.setJvmtiConfig(
          Commands.BeginSession.JvmtiConfig.newBuilder()
            .setAttachAgent(true)
      ...
      Profiler.BeginSessionRequest.Builder requestBuilder = Profiler.BeginSessionRequest.newBuilder()
      ...
      if (device.getFeatureLevel() >= AndroidVersion.VersionCodes.O) {
        ...
        // of re-attaching an agent. See ProfilerService::AttachAgent on the Perfd side for more details.
        requestBuilder.setJvmtiConfig(
          Profiler.BeginSessionRequest.JvmtiConfig.newBuilder()
            .setAttachAgent(true)
            .setAgentLibFileName(String.format("libjvmtiagent_%s.so", process.getAbiCpuArch()))
            // TODO remove hard-coded path by sharing what's used in TransportFileManager
            .setAgentConfigPath("/data/local/tmp/perfd/agent.config")
      ...
      Profiler.BeginSessionResponse response = myProfilers.getClient().getProfilerClient().beginSession(requestBuilder.build());
  ...
  public void endCurrentSession() {
    ...
    if (myProfilers.getIdeServices().getFeatureConfig().isUnifiedPipelineEnabled()) {
      Command command = Command.newBuilder()
        ...
        .setEndSession(Commands.EndSession.newBuilder().setSessionId(profilingSession.getSessionId()))
      ...
      Profiler.EndSessionResponse response = myProfilers.getClient().getProfilerClient().endSession(
        Profiler.EndSessionRequest.newBuilder()
  ...
  public void createImportedSession(@NotNull String sessionName,
                                    @NotNull Common.SessionData.SessionStarted.SessionType sessionType,
