...
import com.tcl.tools.idea.util.StreamQueryUtils;
import com.tcl.tools.profilers.SupportLevel;
import com.tcl.tools.profilers.cpu.CpuProfiler;
import com.tcl.tools.profilers.cpu.CpuProfilerStage;
import com.tcl.tools.profilers.customevent.CustomEventProfiler;
import com.tcl.tools.profilers.customevent.CustomEventProfilerStage;
import com.tcl.tools.profilers.energy.EnergyProfiler;
import com.tcl.tools.profilers.energy.EnergyProfilerStage;
import com.tcl.tools.profilers.event.EventProfiler;
import com.tcl.tools.profilers.memory.MainMemoryProfilerStage;
import com.tcl.tools.profilers.memory.MemoryProfiler;
import com.tcl.tools.profilers.network.NetworkProfiler;
import com.tcl.tools.profilers.network.NetworkProfilerStage;
...
public class StudioProfilers extends AspectModel<ProfilerAspect> implements Updatable {
  ...
  public StudioProfilers(@NotNull ProfilerClient client, @NotNull IdeProfilerServices ideServices, @NotNull StopwatchTimer timer) {
    ...
    myAgentData = Common.AgentData.getDefaultInstance();
    ...
    registerSessionChangeListener(Common.SessionMetaData.SessionType.FULL, () -> {
      ...
      if (SessionsManager.isSessionAlive(mySelectedSession)) {
        ...
        Transport.TimeResponse timeResponse = myClient.getTransportClient().getCurrentTime(
          Transport.TimeRequest.newBuilder().setStreamId(mySelectedSession.getStreamId()).build());
  ...
  public static List<Common.Device> getUpToDateDevices(boolean isUnifiedPipelineEnabled,
    ...
    if (isUnifiedPipelineEnabled) {
      ...
      devices = streams.stream().map((Common.Stream stream) -> {
    ...
    try {
      ...
      if (myIdeServices.getFeatureConfig().isUnifiedPipelineEnabled()) {
        ...
        for (Common.Device device : devices) {
          ...
          Transport.GetProcessesResponse processes = myClient.getTransportClient().getProcesses(request);
      ...
      if (SessionsManager.isSessionAlive(mySelectedSession)) {
        Common.AgentData agentData = getAgentData(mySelectedSession);
        ...
        if (agentData.getStatus() == Common.AgentData.Status.UNSPECIFIED && agentStatusRetryCount >= AGENT_STATUS_MAX_RETRY_COUNT) {
        ...
        if (!myAgentData.equals(agentData)) {
          if (myAgentData.getStatus() != Common.AgentData.Status.ATTACHED &&
              agentData.getStatus() == Common.AgentData.Status.ATTACHED) {
  ...
  private void profilingSessionChanged() {
    ...
    if (!Common.Session.getDefaultInstance().equals(myProfilingSession)
      ...
      if (getAgentData(myProfilingSession).getStatus() == Common.AgentData.Status.ATTACHED) {
        getIdeServices().getFeatureTracker().trackAdvancedProfilingStarted();
      }
    }
  }
  ...
  private Common.AgentData getAgentData(@NotNull Common.Session session) {
    ...
    if (!myIdeServices.getFeatureConfig().isUnifiedPipelineEnabled()) {
      ...
      Transport.AgentStatusRequest statusRequest =
        Transport.AgentStatusRequest.newBuilder().setPid(session.getPid()).setDeviceId(session.getStreamId()).build();
  ...
  public boolean isAgentAttached() {
    return myAgentData.getStatus() == Common.AgentData.Status.ATTACHED;
  }
