 ...
 */
package com.tcl.tools.idea.profilers.perfetto.traceprocessor
...
import com.tcl.tools.idea.profilers.traceprocessor.NameDemangler
import com.tcl.tools.idea.profilers.traceprocessor.NameHolder
import com.tcl.tools.profilers.memory.adapters.ClassDb
import com.tcl.tools.profilers.memory.adapters.NativeAllocationInstanceObject
...
class HeapProfdConverter(
    ...
    fun populateHeapSet(context: NativeAllocationContext) {
        ...
        context.allocationsList.forEach { allocation ->
            ...
            if (callSiteId > 0L) {
                val frameName =
                    pointerMap[callSiteId]?.let { frames[it.frameId]?.last()?.methodName } ?: UNKNOWN_FRAME.methodName
                fullStack.addFrames(
                    0, Memory.AllocationStack.StackFrame.newBuilder()
