 ...
 */
package com.tcl.tools.idea.profilers.traceprocessor
...
import com.tcl.tools.profilers.memory.adapters.classifiers.NativeMemoryHeapSet
import com.tcl.tools.profilers.perfetto.traceprocessor.TraceProcessorModel
...
import com.tcl.tools.idea.profilers.perfetto.traceprocessor.HeapProfdConverter
...
class TraceProcessorServiceImpl(
  ...
  companion object {
    ...
    @VisibleForTesting
    fun buildCpuDataRequestProto(traceId: Long, processIds: List<Int>, selectedProcessName: String = ""): TraceProcessor.QueryBatchRequest {
        ...
        .addQuery(
          TraceProcessor.QueryParameters.newBuilder()
                    ...
                    .setAndroidFrameEventsRequest(
                      TraceProcessor.QueryParameters.AndroidFrameEventsParameters.newBuilder()
      ...
      for (id in processIds) {
        queryBuilder.addQuery(
          TraceProcessor.QueryParameters.newBuilder()
                                .setTraceId(traceId)
                                .setTraceEventsRequest(TraceProcessor.QueryParameters.TraceEventsParameters.newBuilder().setProcessId(id.toLong())))
        queryBuilder.addQuery(
          TraceProcessor.QueryParameters.newBuilder()
                                .setTraceId(traceId)
                                .setProcessCountersRequest(
                                  TraceProcessor.QueryParameters.ProcessCountersParameters.newBuilder().setProcessId(id.toLong())))
      }
      ...
      .addQuery(
        TraceProcessor.QueryParameters.newBuilder()
                  ...
                  .setTraceMetadataRequest(
                    TraceProcessor.QueryParameters.TraceMetadataParameters.newBuilder()
  ...
  private fun executeBatchQuery(traceId: Long,
                                query: TraceProcessor.QueryBatchRequest,
                                ideProfilerServices: IdeProfilerServices): TraceProcessorDaemonQueryResult<TraceProcessor.QueryBatchResponse> {
    ...
    if (queryResult.response?.resultList?.any { it.failureReason == TraceProcessor.QueryResult.QueryFailureReason.TRACE_NOT_FOUND } == true) {
